...
def get_matches():
    base_url = 'https://www.soccerbet.rs/restapi/offer/sr/sport/S/mob'
    params = {
        'annex': 0,
...
async def fetch_and_process(session, match_id, keys):
    ...
    headers = {
        ...
        'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36',
        'Accept': 'application/json, text/plain, */*',
        'Referer': 'https://www.soccerbet.rs/sr/sportsko-kladjenje/fudbal/S/nemacka-1/2515986/special/dortmund-v-bochum/512040854',
...
async def soccer():
    async with aiohttp.ClientSession() as session:
        ...
        if results:
            ...
            df = df.replace(0, 1.0)
            df.to_csv('/content/soccer.csv', index=False, header=head)
